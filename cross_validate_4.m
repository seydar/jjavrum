function [ tfr tsr ] = cross_validate_4(firsts, seconds)
  disp('cross validating conjunctions');
  %db = db_setup('.');
  ...
  for j = 1:size(firsts, 1)
    ...
    first_feats = [first_feats; firsts(j).features.conjunctions];
  end

  for j = 1:size(seconds, 1)
    somme = sum(seconds(j).features.ety);
    second_feats = [second_feats; seconds(j).features.conjunctions];
  end
