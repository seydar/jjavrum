function [ tfr tsr ] = cross_validate_3(firsts, seconds)
  disp('cross validating punctuation');
  %db = db_setup('.');
  ...
  for j = 1:size(firsts, 1)
    ...
    first_feats = [first_feats; firsts(j).features.punctuation];
  end

  for j = 1:size(seconds, 1)
    somme = sum(seconds(j).features.ety);
    second_feats = [second_feats; seconds(j).features.punctuation];
  end
