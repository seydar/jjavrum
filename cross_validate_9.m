function [ tfr tsr ] = cross_validate_9(firsts, seconds)
  disp('cross validating etymologies');
  %db = db_setup('.');
  ...
  for j = 1:size(firsts, 1)
    ...
    first_feats = [first_feats; firsts(j).features.ety./somme];
  end

  for j = 1:size(seconds, 1)
    somme = sum(seconds(j).features.ety);
    second_feats = [second_feats; seconds(j).features.ety./somme];
  end
